...
class ResDoubleConv(nn.Module):
  ...
  def forward(self, x):
    ...
    double_conv = self.conv(x)
    out = self.relu(identity + double_conv)
    return out
...
class AttentionUNET3D(nn.Module):
    def __init__(self, in_channels=1, out_channels=2):  
        ...
        self.conv1 = ResDoubleConv(in_channels, feature_maps[0])
        self.conv2 = ResDoubleConv(feature_maps[0], feature_maps[1])
        self.conv3 = ResDoubleConv(feature_maps[1], feature_maps[2])
        self.conv4 = ResDoubleConv(feature_maps[2], feature_maps[3])

        self.bottleneck = ResDoubleConv(feature_maps[3], feature_maps[4])
        ...
        self.conv4_r = ResDoubleConv(2 * feature_maps[3], feature_maps[3])
        ...
        self.conv3_r = ResDoubleConv(2 * feature_maps[2], feature_maps[2])
        ...
        self.conv2_r = ResDoubleConv(2 * feature_maps[1], feature_maps[1])
        ...
        self.conv1_r = ResDoubleConv(2 * feature_maps[0], feature_maps[0])
...
train_ds = CacheDataset(data=train_files, transform=train_transforms, cache_rate=1.0, num_workers=6)
train_loader = DataLoader(train_ds, batch_size=3, shuffle=True, num_workers=6)

val_ds = CacheDataset(data=val_files, transform=val_transforms, cache_rate=1.0, num_workers=6)
