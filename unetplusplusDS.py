...
class UNetPlusPlus3D(nn.Module):
    def __init__(self, in_channels=1, out_channels=3, deep_supervision=True):
...
model = UNetPlusPlus3D(in_channels=1, out_channels=3, deep_supervision=True).to(device)
...
log_filename = "/workspace/unetplusplus_DS_training_log.csv"
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            if metric > best_metric:
                ...
                torch.save(model.state_dict(), "/workspace/unetplusplus_DS_best_metric_model.pth")
